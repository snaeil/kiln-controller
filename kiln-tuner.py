...
def recordprofile(csvfile, targettemp):
    ...
    try:
        ...
        while temp <= targettemp:
            ...
            print(
                "stage = %s, actual = %.2f, target = %.2f" % (stage, temp, targettemp)
            )
...
def plot(
    ...
    pyplot.plot(
        [minx, maxx],
        [
            line(tangent_slope, tangent_offset, minx),
            line(tangent_slope, tangent_offset, maxx),
        ],
        "--",
        color="red",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--target_temp", type=float, default=400, help="Target temperature"
    )
    parser.add_argument(
        "-d",
        "--tangent_divisor",
